...
import { analyzeAriaHiddenFocusable } from './analyzers/aria-hidden-focusable';
import {
...
import { analyzeWithAxeCore } from './analyzers/axe-core';
import { analyzeContentGrouping } from './analyzers/content-grouping';
import { analyzeEmptyAccessibleNames } from './analyzers/empty-accessible-name';
import { analyzeFocusOrder } from './analyzers/focus-order';
import { analyzeFocusTraps } from './analyzers/focus-trap';
import { analyzeFormLabels } from './analyzers/form-labels';
import { analyzeHeadingStructure } from './analyzers/heading-structure';
import { analyzeImageAltText } from './analyzers/image-alt-text';
import { analyzeKeyboardAccessibility } from './analyzers/keyboard-accessibility';
import { analyzeLandmarkStructure } from './analyzers/landmark-structure';
import { analyzeLinkText } from './analyzers/link-text';
import { analyzeNavigationSize } from './analyzers/navigation-size';
import { analyzeRoleMismatch } from './analyzers/role-mismatch';
import { analyzeSkipLink } from './analyzers/skip-link';
...
export function summarizeViolations(violations: readonly Violation[]): ViolationTotals {
    ...
    for (const violation of violations) {
        ...
        totals.byImpact[violation.impact] = (totals.byImpact[violation.impact] ?? 0) + 1;
        totals.byRule[violation.ruleId] = (totals.byRule[violation.ruleId] ?? 0) + 1;
    }

...
const strategies = (linkSteps: NavigationStep[], arrowSteps?: NavigationStep[]): AuditContext => {
...
describe('analyzeLinkText generic link text', () => {
    it('reports a generic link at full impact when the arrow walk found no context', async () => {
        const arrowSteps = [
        ...
        const result = await analyzeLinkText(strategies([genericLink], arrowSteps));

        expect(result.violations).toHaveLength(1);
        expect(result.violations[0]).toMatchObject({ rule: { id: 'generic-link-text', impact: 'serious' } });
    ...
    it('downgrades a generic link that sits in surrounding text and quotes that text', async () => {
        const arrowSteps = [
        ...
        expect(result.violations).toHaveLength(1);
        expect(result.violations[0]).toMatchObject({ rule: { id: 'generic-link-text', impact: 'moderate' } });
    ...
    it('falls back to full impact when the arrow walk never reached the link', async () => {
        const arrowSteps = [createStep(0, { itemText: 'Some other content' })];

        const result = await analyzeLinkText(strategies([genericLink], arrowSteps));

        expect(result.violations[0]).toMatchObject({ rule: { impact: 'serious' } });
    });

    it('falls back to full impact when no arrow strategy ran at all', async () => {
        const result = await analyzeLinkText(strategies([genericLink]));
    ...
    it('leaves descriptive link text alone', async () => {
        ...
        const result = await analyzeLinkText(strategies([descriptive]));

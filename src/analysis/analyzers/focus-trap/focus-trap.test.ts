...
describe('analyzeFocusTraps', () => {
    it('reports the element the tab walk got stuck on', async () => {
        ...
        const result = await analyzeFocusTraps(mockContext([strategyResult('tab', steps, trapped)]));

        expect(result.violations).toHaveLength(1);
        expect(result.violations[0]).toMatchObject({
            rule: { id: 'focus-trap', impact: 'critical', wcag: { primary: { criterion: '2.1.2', level: 'A' } } },
        ...
        expect(result.violations[0]?.context).toMatchObject({ step: { strategy: 'tab', index: 2 } });
    });

    it('stays silent when the tab walk completed its cycle', async () => {
        const steps = createSteps([{ itemText: 'Home' }, { itemText: 'Plans' }]);

        const result = await analyzeFocusTraps(mockContext([strategyResult('tab', steps, cycleComplete)]));

        expect(result.violations).toEqual([]);
        expect(result.summary).toEqual({ tabStrategiesChecked: 1, focusTrapsFound: 0 });
    });

    it('ignores a trapped completion reported by a browse-mode strategy', async () => {
        const steps = createSteps([{ itemText: 'Home' }]);

        const result = await analyzeFocusTraps(mockContext([strategyResult('arrow', steps, trapped)]));
    ...
    it('has nothing to report when the trapped run recorded no steps', async () => {
        const result = await analyzeFocusTraps(mockContext([strategyResult('tab', [], trapped)]));
    ...
    it('checks every tab strategy in the run', async () => {
        ...
        const result = await analyzeFocusTraps(

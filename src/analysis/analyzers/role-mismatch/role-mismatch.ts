...
export async function analyzeRoleMismatch(ctx: AuditContext): Promise<RoleMismatchAnalyzerResult> {
    ...
    for (const element of uniqueElements) {
        ...
        if (mismatch) {
            ...
            violations.push(createMismatchViolation(element, mismatch, context));
        }
    }
...
function createMismatchViolation(element: ElementInfo, mismatch: MismatchInfo, context: NvdaContext): NvdaViolation {
    return {
        id: `role-mismatch-${element.identifier}`,
        rule: getRule('role-mismatch'),

 ...
 *
 * Analyzers all consume the same `StrategyResult[]` shape, so the builders
 ...
 *
 * Used by the suite-wide guards that assert a property of *every* analyzer
 * rather than one analyzer's logic. Those guards are only as good as the number

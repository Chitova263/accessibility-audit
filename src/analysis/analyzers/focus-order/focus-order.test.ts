...
const strategies = (tabSteps: NavigationStep[], arrowSteps?: NavigationStep[]): AuditContext => {
...
describe('analyzeFocusOrder reading order comparison', () => {
    it('stays silent when tab order follows reading order', async () => {
        ...
        const result = await analyzeFocusOrder(strategies(tabSteps, readingWalk()));
    ...
    it('reports focus jumping backwards through the reading order', async () => {
        ...
        expect(result.violations.map((v) => v.rule.id)).toEqual(['focus-order-anomaly']);
    ...
    it('tolerates small backwards movement from imprecise reading-order matching', async () => {
        ...
        const result = await analyzeFocusOrder(strategies(tabSteps, arrowSteps));
    ...
    it('skips reading-order comparison when no arrow strategy ran', async () => {
        ...
        const result = await analyzeFocusOrder(strategies(tabSteps));

        expect(result.summary.elementsWithReadingPosition).toBe(0);
        expect(result.violations).toEqual([]);
    });

    it('still reports positive tabindex without any reading order', async () => {
        const tabSteps = [createStep(0, { htmlSnippet: '<a href="/a" tabindex="3">Plans</a>' })];

        const result = await analyzeFocusOrder(strategies(tabSteps));

        expect(result.violations.map((v) => v.rule.id)).toEqual(['positive-tabindex']);
    });
});

...
describe('analyzeImageAltText', () => {
    it.each([
        ...
        const result = await analyzeImageAltText(mockContext([strategyResult('arrow', createSteps([image(alt)]))]));

        expect(result.violations).toHaveLength(1);
        expect(result.violations[0]).toMatchObject({
            rule: { id: 'filename-as-alt', impact: 'serious', wcag: { primary: { criterion: '1.1.1', level: 'A' } } },
        });
    ...
    it('does not report a decorative image with an empty alt', async () => {
        const result = await analyzeImageAltText(mockContext([strategyResult('arrow', createSteps([image('')]))]));
    ...
    it('recognises an image from its HTML even when the role says otherwise', async () => {
        ...
        const result = await analyzeImageAltText(mockContext([strategyResult('link', steps)]));
    ...
    it('counts an image once when several strategies reached it', async () => {
        ...
        const result = await analyzeImageAltText(
    ...
    it('reports each offending image separately', async () => {
        ...
        const result = await analyzeImageAltText(mockContext([strategyResult('arrow', steps)]));

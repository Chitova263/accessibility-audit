...
describe('analyzeKeyboardAccessibility', () => {
    it('reports a button reachable with B but absent from the tab order', () => {
        const result = analyzeKeyboardAccessibility({
            strategyResults: [
    ...
    it('reports a link reachable with K but absent from the tab order', () => {
        const result = analyzeKeyboardAccessibility({
            strategyResults: [
        ...
        expect(result.violations.map((v) => v.ruleId)).toEqual(['link-not-in-tab-order']);
        expect(result.violations[0]?.message).toContain('reachable via K key navigation');
    });

    it('stays silent when the same element appears in the tab order', () => {
        const result = analyzeKeyboardAccessibility({
            strategyResults: [
                strategyResult('button', createSteps([button('Submit')])),
    ...
    it('matches loosely when the tab announcement carries extra text', () => {
    ...
    it('reports an unnamed button, which cannot be matched loosely', () => {
        const result = analyzeKeyboardAccessibility({
            strategyResults: [
    ...
    it('only collects elements whose role matches the strategy that found them', () => {
        const result = analyzeKeyboardAccessibility({
            strategyResults: [
    ...
    it('counts elements found in browse mode and in focus mode', () => {
        const result = analyzeKeyboardAccessibility({
            strategyResults: [

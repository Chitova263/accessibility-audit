...
export class HeadingNavigationStrategy implements INavigationStrategy {
    ...
    public async execute(
        ...
        for (let steps = 0; steps < this.config.maxSteps; steps++) {
            ...
            await sr.nextHeading();

            const spokenPhrases = await sr.spokenPhraseLog();
            const itemText = await sr.itemText();

            // NVDA announces "no next heading" when there are no more headings to navigate to
            const noMoreHeadings = spokenPhrases.some((p) => p.toLowerCase().includes('no next heading'));
            if (noMoreHeadings) {
                return {

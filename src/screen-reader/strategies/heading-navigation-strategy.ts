...
export class HeadingNavigationStrategy implements NavigationStrategy {
    public readonly meta: NavigationStrategyMetadata = {
        name: 'heading',
        description: 'Navigates through all headings using browse mode heading navigation',
    };
    ...
    public async execute(ctx: NavigationContext): Promise<StrategyResult> {
        ...
        for await (const { phrase, focusedElementText } of ctx.virtualCursor.headings()) {

...
```mermaid
flowchart LR
    URL[URL Input] --> AUDIT[ally audit]
    AUDIT --> FILES[Audit Files]
    FILES --> LLM[LLM Analysis]
    LLM --> REPORT[ally report]
...
**Command:** `ally audit <url> [options]`
...
flowchart TB
    ...
    subgraph DriverFactory["createDriver()"]
        ...
        VIRTUAL["Virtual Driver<br/>• Headless<br/>• Cross-platform<br/>• Chrome DevTools Protocol"]
    end
...
**Command:** `ally report --dir <audit-dir>`
